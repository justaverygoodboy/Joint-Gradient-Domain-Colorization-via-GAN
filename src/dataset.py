...
class DATA():
    ...
    def read_img(self, filename): #读取图片
        img = cv2.imread(filename, 3) 
        height, width, channels = img.shape
        min_hw = int(min(height,width)/2)
        img = img[int(height/2)-min_hw:int(height/2)+min_hw,int(width/2)-min_hw:int(width/2)+min_hw,:] #这个是干嘛
        labimg = cv2.cvtColor(cv2.resize(img, (config.IMAGE_SIZE, config.IMAGE_SIZE)), cv2.COLOR_BGR2LAB) #这里应该是BGR2LAB吧，原本作者写的RGB
